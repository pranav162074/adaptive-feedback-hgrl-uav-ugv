...
waypoints = [2,0;3,1;5,3;2,4;0.5,2;0,8;2,9;8,9.5;6,0;6,4;9,5;10,3;1,6;5,6;7.5,1;3,7;9.5,7.5;6.5,7;4,4.5;]; % 必经点
...
for gen = 1:max_gen    
    ...
    for i=1:pop_size
     fitness(i) = all_total_distance(pop(i, :),points,param , n_travelers,alpha,beta);
    end
    % 适应度归一化，越大越好
%     fitness = 1 ./ (1 + fitness);
    % 选择操作（轮盘赌选择）
...
% 获取最优路径
best_path1 = pop(best_idx, :);
optimalPath = points(best_path1, :);
best_path_length=best_fitness;
% 输出最优路径和最短距离
disp('最短路径:');
disp(points(best_path1, :));
disp(best_path1);
    ...
    for i = 1:num_points-1
     x_start = current_points(i, 1);
      ...
      if distMatrix(fullOrder(i),fullOrder(i+1))~=0
             ...
             y_path=[];
             for j=1:size(xMatrix1,3)
                 if xMatrix1(fullOrder(i),fullOrder(i+1),j)==0 && yMatrix1(fullOrder(i),fullOrder(i+1),j)==0&&j~=1&&x_path(1)~=0&&y_path(1)~=0
                     break;
                 else
              x_path(j)=xMatrix1(fullOrder(i),fullOrder(i+1),j);
               y_path(j)=yMatrix1(fullOrder(i),fullOrder(i+1),j);
...
%     distMatrix(fullOrder(i+1),fullOrder(i))=path_length;
    for j=1:length(x_path)
     xMatrix1(fullOrder(i),fullOrder(i+1),j)=x_path(j);
    yMatrix1(fullOrder(i),fullOrder(i+1),j)=y_path(j);
    end
      end
       all_x = [all_x, x_path(2:end)];
    all_y = [all_y, y_path(2:end)];  
...
function len = all_total_distance(path,points,param , n_travelers,alpha,beta)
    len = 0;
