...
for gen = 1:max_gen
    ...
    for i = 1:pop_size
        fitness(i) = total_distance(pop(i, :), dist_matrix, n_travelers,near_zone,alpha,beta);
    end
...
% 计算路径总长度的函数
function len = total_distance(path, dist_matrix, n_travelers,near_zone,alpha,beta)
    len = 0;
    max_len=0;
     traveleridx=find(path<=n_travelers,n_travelers);
    for t = 1:n_travelers
       traveler_path=travelers_path_func(path, dist_matrix, n_travelers,near_zone,traveleridx,t);
       len1=0;
